...
import { RequestedThemeOptions, TerminalKeyBinding, NavLinkPaths, StoreType } from '../types/types';
...
const ConfigGlobalsPage = () => {
    const {globals, profiles, terminalConfig} = store.getState()
    const {setGlobals, setSpecificKeyBinding} = store.getActions()
    console.log(globals)
            ...
            <Container>
                ...
                <Divider />
                {
                    ...
                    <>
                        ...
                        <Dropdown button selection fluid labeled floating icon='setting' className='icon' text={'Default Profile : ' + (profiles.filter((e :any)=> e.guid === globals.defaultProfile)[0].name)}
                        ...
                        {
                            globals.keybindings.map((k:any) =>
                                (
                                    <KeyBindingBar key={k.command} keybinding={k} setKeybinding={setSpecificKeyBinding} />
                                )
                            )
                        }
